...
class VP_Wallet {
     ...
     */
    /**
     * واریز اعتبار به کیف پول کاربر (ایمن‌شده با Transaction)
     */
    public function credit(
        ...
        // --- شروع تراکنش و قفل ایمن ---
        $wpdb->query( 'START TRANSACTION' );
        $wpdb->query( $wpdb->prepare( "SELECT ID FROM {$wpdb->users} WHERE ID = %d FOR UPDATE", $user_id ) );

        // محاسبه موجودی به‌صورت زنده (بعد از قفل شدن ردیف دیتابیس)
        $balance_before = $this->calculate_balance_from_db( $user_id );
        ...
        if ( $result ) {
            $wpdb->query( 'COMMIT' ); // ثبت نهایی در دیتابیس و باز کردن قفل
            
            if ( $status === 'active' ) {
                update_user_meta( $user_id, self::META_BALANCE, $balance_after ); // آپدیت کش
            }
        ...
        $wpdb->query( 'ROLLBACK' ); // لغو در صورت بروز خطای دیتابیس
        return false;
    }
     ...
     */
    /**
     * برداشت اعتبار از کیف پول (کاملاً ایمن در برابر Race Condition)
     */
    public function debit(
        ...
        // شروع تراکنش و قفل کردن ردیف کاربر برای جلوگیری از تغییر همزمان موجودی
        $wpdb->query( 'START TRANSACTION' );
        $wpdb->query( $wpdb->prepare( "SELECT ID FROM {$wpdb->users} WHERE ID = %d FOR UPDATE", $user_id ) );

        // خواندن موجودی به‌صورت زنده از دیتابیس (بدون استفاده از Cache در لحظه‌ی تراکنش)
        $balance = $this->calculate_balance_from_db( $user_id );
    ...
    /**
     * کسر موجودی واقعی بعد از تکمیل سفارش (ایمن در برابر کسر تکراری)
     */
    public function deduct_on_order_complete( int $order_id ): void {
        ...
        // بررسی اینکه آیا قبلاً این مبلغ کسر شده است؟ (جلوگیری از Double Deduction)
        if ( $order->get_meta( '_vp_wallet_deducted' ) === 'yes' ) {
            return;
        }
        ...
        // تنها در صورتی نشانه‌گذاری کن که برداشت موفق باشد
        if ( $debit_success ) {
            $order->update_meta_data( '_vp_wallet_deducted', 'yes' );
